<div class="card">
    <div class="card-header">
        <a href="index.php?act=hotel&pg=hotel_add" class="btn btn-primary"><i class="fas fa-plus"></i> เพิ่มข้อมูล</a>
    </div>
    <div class="card-body">
...
<script>
    $(document).ready(function() {
      ...
      $('#hotelTable').dataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": "admin_hotel/hotel_list_ajax.php",
                "type": "POST"
            },
            // Thai lang
            "oLanguage": {
    ...
    async function appChange(param1, param2, id) {
        if ($(`#${param1}`).prop("checked")) {
            $(`#${param2}`).text('อนุมัติ');
            try {
                const response = await axios.get(`admin_hotel/hotel_status_ajax.php?id=${id}&status=1`);
            ...
            try {
                const response = await axios.get(`admin_hotel/hotel_status_ajax.php?id=${id}&status=2`);
                $('#hotelTable').DataTable().ajax.reload();
                console.log(response.data);
            } catch (error) {
                console.error(error);
